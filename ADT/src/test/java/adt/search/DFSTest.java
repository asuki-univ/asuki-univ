...
import adt.search.DepthFirstSearch;

public abstract class DFSTest {
    private DepthFirstSearch dfs;
    
    protected DFSTest(DepthFirstSearch dfs) {
        this.dfs = dfs;
    }
    ...
    public void testFindSingle() {
        Graph g = new Graph(1);
        Vertex[] iters = toArray(dfs.iter(g, g.vertex(0)));
    ...
    public void testFind7() {
        Graph g = new Graph(7);
        g.addEdge(new Edge(g.vertex(0), g.vertex(1))); {
            g.addEdge(new Edge(g.vertex(1), g.vertex(2)));
            g.addEdge(new Edge(g.vertex(1), g.vertex(3)));
        }
        g.addEdge(new Edge(g.vertex(0), g.vertex(4))); {
            g.addEdge(new Edge(g.vertex(4), g.vertex(5)));
            g.addEdge(new Edge(g.vertex(4), g.vertex(6)));
        }
